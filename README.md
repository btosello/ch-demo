...
└──────────────────────────────────────────────────────────────┼────┼────┼───┼────┘
                    ...
                    ▼                              ▼                              ▼
    ┌───────────────────────────┐  ┌───────────────────────────┐  ┌───────────────────────────┐
    │     BUILD DOCKER IMAGE    │  │   VALIDATE K8S MANIFESTS  │  │    (parallel)             │
    │  • Setup Docker Buildx    │  │  • Install kustomize      │  │                           │
    │  • Login to GHCR          │  │  • Install kubeconform    │  │                           │
    │  • Build multi-arch       │  │  • Validate base          │  │                           │
    │    (amd64 + arm64)        │  │  • Validate local overlay │  │                           │
    │  • Push with tags         │  │  • Validate prod overlay  │  │                           │
    │  • Generate SBOM          │  │                           │  │                           │
    └─────────────┬─────────────┘  └───────────────────────────┘  └───────────────────────────┘
                  │
                  ▼
...
│  (only on staging branch)       │     │      (only on v* tags)          │
│  • Update image tag             │     │  • Update image tag             │
│  • Deploy to staging cluster    │     │  • Deploy to prod cluster       │
│  • Verify rollout               │     │  • Verify rollout               │
│  • Environment: staging         │     │  • Run smoke tests              │
...
| Event | Branches/Tags | Jobs Executed |
|-------|---------------|---------------|
| Push | `main` | Build, Test, Image, Scan, Validate |
| Push | `staging` | Build, Test, Image, Scan, Validate, **Deploy Staging** |
| Push Tag | `v*` | Build, Test, Image, Scan, Validate, **Deploy Prod**, **Release** |
| Pull Request | `main` | Build, Test, Image (no push), Validate |
...
│                                                                          │
│  Push to main:                                                           │
│    ghcr.io/btosello/ch-demo/websocket-server:main                       │
│    ghcr.io/btosello/ch-demo/websocket-server:sha-abc1234                │
│    ghcr.io/btosello/ch-demo/websocket-server:latest                     │
│                                                                          │
│  Push to staging:                                                        │
│    ghcr.io/btosello/ch-demo/websocket-server:staging                    │
│    ghcr.io/btosello/ch-demo/websocket-server:sha-def5678                │
│                                                                          │
│  Tag v1.2.3:                                                             │
│    ghcr.io/btosello/ch-demo/websocket-server:v1.2.3     (full version)  │
│    ghcr.io/btosello/ch-demo/websocket-server:1.2.3      (semver)        │
│    ghcr.io/btosello/ch-demo/websocket-server:1.2        (major.minor)   │
│    ghcr.io/btosello/ch-demo/websocket-server:1          (major only)    │
│    ghcr.io/btosello/ch-demo/websocket-server:sha-ghi9012                │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```
...
# 5. Approve the production deployment in GitHub Environments (if required)
# 6. Verify the GitHub Release was created automatically
```
...
|-------------|-----------------|
| `staging` | None (auto-deploy on push to staging branch) |
